...
def slider(na,defa,ls,event):
    return sg.Slider(range=(ls[0],ls[1]),  key=na,default_value=defa,resolution=-100, tick_interval=1,pad= None,orientation='h',disable_number_display=False, enable_events=event)
def checkBox(na,ky,event,defa,state):
    return sg.Checkbox(na, enable_events=event, key=ky,default=defa,disabled=state, size=(1, 0),pad=(0,0))
def checkBoxStnd(ky,event,defa,state):
    return sg.Checkbox('',key=ky,enable_events=event,default=defa,disabled=state,size=(1, 0))
...
def getFullSlider(na,defa,ls,event,opt):
    return [checkBoxStnd(na+'default_'+str(defa),True,True,opt),checkBoxStnd(na+'disable',True,True,opt),button(na,na+'_info',True),slider(na,defa,ls,event)]
...
def desktopTheme():
  ...
  window = sg.Window('Dashboard PySimpleGUI-Style', layout, margins=(5,5), element_padding=(5,5), background_color=templateJs['BORDER_COLOR'], keep_on_top=False, no_titlebar=True, resizable=True, right_click_menu=sg.MENU_RIGHT_CLICK_EDITME_VER_LOC_EXIT)
  while True:
        event, values = window.read()
        ...
        if event == 'SEND':
            ...
            window.refresh()
  return values
global category,specializations,categories,specialization,catKeys,specializedSet,catLs,categoriesJs,specializations,info,tabIndex,parameters
...
specializedSet = {'notFound':[],'fileUps':[],'inputTabKeys':{'types':[],'inputLs':[],'index':1,'names':[],'descriptions':[]},'jsList':[[sg.Text('Parameters', font='Any 20')],[txtBox('def  ',None,'Any 20',None,True,False),txtBox('dis  ',None,'Any 20',None,True,False),txtBox('   inf  ',None,'Any 20',None,True,False)]],'prevKeys':[],'userMgs':'','resp':'','content':'application/json'}
